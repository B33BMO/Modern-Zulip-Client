...
// electron/main.ts
var state = { baseUrl: "", email: "" };
var SERVICE_API = (base) => `zulip-glass:${base}`;
var SERVICE_COOKIE = (base) => `zulip-glass-cookie:${base}`;
function normBase(u) {
  let s2 = u.trim();
...
function basic(email, key) {
  const tok = import_node_buffer3.Buffer.from(`${email}:${key}`).toString("base64");
  return `Basic ${tok}`;
}
function isAssetPath(p) {
  return p.startsWith("/user_uploads/") || p.startsWith("/user_avatars/") || p.startsWith("/thumbnail/") || p.startsWith("/avatar/") || p.startsWith("/external_content/") || p.startsWith("/static/");
}
function toAbsolute(base, pathOrAbs) {
  if (/^https?:\/\//i.test(pathOrAbs)) return pathOrAbs;
  const p = pathOrAbs.startsWith("/") ? pathOrAbs : `/${pathOrAbs}`;
  return `${base}${p}`;
}
import_electron.ipcMain.handle("zulip:loginApiKey", async (_e, { email, apiKey, baseUrl }) => {
  ...
  await import_keytar.default.setPassword(SERVICE_API(state.baseUrl), email, apiKey);
  state.apiKey = apiKey;
  try {
    state.cookie = await import_keytar.default.getPassword(SERVICE_COOKIE(state.baseUrl), "session") || "";
  } catch {
  }
  return true;
});
import_electron.ipcMain.handle("zulip:loginPassword", async (_e, { email, password, baseUrl }) => {
  state.baseUrl = normBase(baseUrl);
  state.email = email;
  const loginPaths = ["/accounts/login/local/", "/accounts/login/"];
  let loginHtml = "", setCookies = [], loginPath = loginPaths[0];
  for (const p of loginPaths) {
    const r2 = await fetch(`${state.baseUrl}${p}`, { redirect: "manual" });
    if (r2.ok) {
      loginHtml = await r2.text();
      setCookies = r2.headers.raw()["set-cookie"] || [];
      loginPath = p;
      break;
    }
  }
  if (!loginHtml) throw new Error("Unable to load login page");
  const cookieStr = setCookies.find((c) => c.toLowerCase().startsWith("csrftoken=")) || setCookies.find((c) => c.toLowerCase().startsWith("csrf="));
  let csrftoken = "";
  if (cookieStr) csrftoken = cookieStr.split(";")[0].split("=")[1] || "";
  if (!csrftoken) {
    const m2 = loginHtml.match(/name="csrfmiddlewaretoken"\s+value="([^"]+)"/i);
  ...
  if (cookieStr) headers.set("cookie", cookieStr.split(";")[0]);
  ...
  const sessionCookie = (setCookies2.find((c) => c.toLowerCase().startsWith("sessionid=")) || setCookies2.find((c) => c.toLowerCase().startsWith("sessionid_secure=")))?.split(";")[0];
  if (!sessionCookie) throw new Error("Login did not return a session cookie");
  state.cookie = `${sessionCookie}; csrftoken=${csrftoken}`;
  await import_keytar.default.setPassword(SERVICE_COOKIE(state.baseUrl), "session", state.cookie);
  return true;
});
import_electron.ipcMain.handle("zulip:logout", async (_e, { baseUrl, email }) => {
  const b = normBase(baseUrl);
  try {
    await import_keytar.default.deletePassword(SERVICE_API(b), email);
  } catch {
  }
  try {
    await import_keytar.default.deletePassword(SERVICE_COOKIE(b), "session");
  } catch {
  }
  if (state.baseUrl === b) Object.assign(state, { baseUrl: "", email: "", apiKey: "", cookie: "" });
  return true;
});
import_electron.ipcMain.handle("zulip:get", async (_e, pathAndQuery) => {
  ...
  if (isApi) {
    if (!state.apiKey) {
      const ak = await import_keytar.default.getPassword(SERVICE_API(state.baseUrl), state.email);
...
import_electron.ipcMain.handle("zulip:postForm", async (_e, { path: path2, form }) => {
  if (!state.baseUrl || !state.email) throw new Error("Not logged in");
  if (!state.apiKey) {
    const ak = await import_keytar.default.getPassword(SERVICE_API(state.baseUrl), state.email);
    if (!ak) throw new Error("Missing API key");
  ...
  const url = toAbsolute(state.baseUrl, path2);
...
import_electron.protocol.registerSchemesAsPrivileged([{ scheme: "app-proxy", privileges: { standard: true, secure: true, supportFetchAPI: true, stream: true } }]);
async function registerAppProxy() {
  import_electron.protocol.registerStreamProtocol("app-proxy", async (request, callback) => {
    try {
      if (!state.baseUrl || !state.email) return callback({ statusCode: 401, data: null });
      const after = request.url.replace(/^app-proxy:\/\//, "");
      const path2 = after.startsWith("/") ? after : `/${after}`;
      const url = toAbsolute(state.baseUrl, path2);
      const headers = new Headers();
      if (isAssetPath(path2)) {
      ...
      const statusCode = r2.status;
      const data = r2.body;
      callback({ statusCode, headers: { "Content-Type": ct }, data });
    } catch (e2) {
      console.warn("app-proxy error", e2);
      callback({ statusCode: 404, data: null });
    }
  });
}
async function createWindow() {
  const preloadPath = import_node_path.default.join(import_electron.app.getAppPath(), ".electron/dist/preload.cjs");
